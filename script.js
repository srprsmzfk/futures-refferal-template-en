...
function generateCard() {
    ...
    let img = new Image();
    img.src = `assets/background${fields.background.value}.png`;
    img.onload = () => {
        ...
        drawText(fields['sell-buy'].value, fields['sell-buy'].value === 'Short' ? 135 : 140, 213, `400 30px ${plex}`, fields['sell-buy'].value === 'Short' ? red : green );

        drawText(fields.xxx.value, fields.xxx.value.length === 3 ? 280 : 270, 213, `400 30px ${plex}`, white);

        drawText(fields.coin.value.toUpperCase() + '  Perpetual', 398, 213, `400 30px ${plex}`);

        drawText(`${getPercentValue(fields.percent.value) > 0 ? '+' : ''}${fields.percent.value} %`, 115, 315, `400 95px ${plexCondensed}`, getPercentValue(fields.percent.value) > 0 ? green : red);
